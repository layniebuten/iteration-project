...
} // cents stay stable so totals and settlements do not drift
function sumPays(pays: Pay[]) {
  let sum = 0;
  for (const pay of pays) sum += Number(pay.amount) || 0;
...
const Details = ({ tripID, onGoToRequests }: Props) => {
  ...
  function toggleExp(expId: number) {
    setOpenExp((prev) => ({ ...prev, [expId]: !prev[expId] }));
  }
  function togglePerson(name: string) {
    setOpenPerson((prev) => ({ ...prev, [name]: !prev[name] }));
  }
  ...
  function calcSettle() {
    ...
    const tripTotal = roundCents(
      tripExps.reduce((sum, row) => sum + Number(row.total || 0), 0)
    );
    ...
    for (const expRow of tripExps)
      for (const pay of expRow.payments)
        if (paid[pay.name] !== undefined)
          paid[pay.name] = roundCents(paid[pay.name] + Number(pay.amount || 0));

    // net  -->  positive: overpaid (creditors)  ;  negative: underpaid (debtors)
    const net: Record<string, number> = {};
    for (const name of trip.people)
      net[name] = roundCents((paid[name] || 0) - share);
    const creditors: { name: string; amount: number }[] = [];
    const debtors: { name: string; amount: number }[] = [];
    for (const [name, netamount] of Object.entries(net)) {
      if (netamount > 0)
        creditors.push({ name, amount: roundCents(netamount) });
      if (netamount < 0)
        debtors.push({ name, amount: roundCents(Math.abs(netamount)) });
    }
  ...
  const budgetText =
    budgetDelta > 0
      ? `Over Budget:  $${roundCents(budgetDelta).toFixed(2)}`
      : `Under Budget:  $${roundCents(Math.abs(budgetDelta)).toFixed(2)}`;
  ...
  function rowsFor(name: string) {
    ...
    for (const row of settles) {
      if (row.to === name)
        rows.push({ text: `${row.from} owes ${name}`, amount: row.amount });
      if (row.from === name)
        rows.push({ text: `${name} owes ${row.to}`, amount: row.amount });
    }
    return rows;
  }
  ...
  function submitExp() {
    ...
    if (!Number.isFinite(total) || total <= 0)
      nextErrs.push('Amount must be a number greater than 0.');
    ...
    if (pays.length === 0)
      nextErrs.push('At least one member must pay a positive amount.');
    const payTotal = sumPays(pays);
    if (
      Number.isFinite(total) &&
      pays.length > 0 &&
      roundCents(payTotal) !== roundCents(total)
    )
      nextErrs.push(
        `Amount Total Error: \n    payments sum = $${roundCents(
          payTotal
        ).toFixed(2)}`
      );
    ...
    const maxId = exps.reduce(
      (maxSoFar, row) => Math.max(maxSoFar, row.id),
      -1
    );
    const newExp: Exp = {
      id: maxId + 1,
    ...
    panel = (
      <>
        <header className="panel-head">
          <h3 id="add-expense-title">Add Expense</h3>
        </header>
        {errs.length > 0 && (
          <div id="form-errors">
            <p>Fix these:</p>
        ...
        <div className="form-actions">
          ...
          <button type="button" id="submit-expense" onClick={submitExp}>
            Add Expense
          </button>
        </div>
      </>
      ...
      <>
        <header className="panel-head">
          ...
          <button
            ...
            id="go-requests"
            onClick={() => {
              if (onGoToRequests) onGoToRequests();
            }}
          >
            Requests
          </button>
        ...
        <section id="trip-breakdown">
          <p id="trip-total">
            Trip Total: ${roundCents(tripTotal).toFixed(2)} | {budgetText}
          </p>
              ...
              <div key={name} className="person-row">
                ...
                {isOpen && (
                  <div className="person-details">
                    <p>
                      paid: ${roundCents(paid[name] || 0).toFixed(2)} | net: $
                      {roundCents(net[name] || 0).toFixed(2)}
                    </p>
                    {rows.length === 0 ? (
                      <p>No balances for {name}.</p>
                    ) : (
                      <ul className="person-rows">
                        {rows.map((row) => (
                          <li key={`${name}-${row.text}-${row.amount}`}>
                            {row.text} ${roundCents(row.amount).toFixed(2)}
                          </li>
                        ))}
                      </ul>
                    )}
                    ...
                    >
                      <div className="expense-top">
                        ...
                        <div className="expense-amount">
                          <p>${roundCents(expRow.total).toFixed(2)}</p>
                        </div>
                      </div>
                    ...
                    {isOpen && (
                      <div className="expense-details">
                        <ul className="expense-pays">
                          {expRow.payments.map((pay) => (
                            <li key={`${expRow.id}-${pay.name}`}>
                              {pay.name} → ${roundCents(pay.amount).toFixed(2)}
                            </li>
                          ))}
                        </ul>
                      </div>
  ...
  return (
    <main id="details-page">
      ...
      <section id="details-panel">{panel}</section>
    </main>
  );
};
